...
description: |
  ...
  既存メソッド（集合分析→分類→理由考察→個別ボード→ストリート分岐）に準拠。
...
### Step 5: 個別ボードのFlop戦略を詳細分析（既存メソッド ④）
...
### Step 6: Flop分岐ごとにTurn戦略を分類（既存メソッド ⑤⑥）
