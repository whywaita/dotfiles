...
**PRIMARY DATA SOURCE** — gtowizard CLI に代わるメインのデータ取得手段。

CDP (Chrome DevTools Protocol) の `Fetch` ドメインを使い、GTO Wizard の内部 API レスポンスを直接インターセプトしてソルバーデータを取得する。

## 概要

ブラウザが GTO Wizard のソリューションページを読み込む際に発行される `spot-solution` API コールをインターセプトし、レスポンス JSON をそのまま取得する。取得される JSON は gtowizard CLI の出力と同一フォーマット（[solution-format.md](gtowizard-cli/solution-format.md) 参照）。
...
URL パラメータの仕様は既存の [poker-scenarios.md](gtowizard-cli/poker-scenarios.md) と同一。
...
6. JSON パース → gtowizard CLI と同一フォーマット
...
## Progressive Bet Size Discovery（ブラウザ版）

CLI 版と同じ原則だが、ブラウザ上で実行する:
...
**CLI 版との違い:** ベットサイズの発見にゲームツリーの DOM を直接参照できるため、事前にクエリを発行する必要がない。

## レスポンスフォーマット

取得される JSON は gtowizard CLI の出力と完全に同一。詳細は [solution-format.md](gtowizard-cli/solution-format.md) を参照。
...
- **サブスクリプション権限の完全な利用**: CLI は 30bb で制限されることがあるが、ブラウザ経由なら制限なし
- **スクリーンショットの同時取得**: データ取得と同じフローで画面キャプチャ可能
- **ゲームツリーの DOM ナビゲーション**: ベットサイズの発見がより直感的
- **Go バイナリ不要**: 別途のバイナリインストールが不要
