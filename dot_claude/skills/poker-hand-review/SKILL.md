...
7. **スクリーンショット取得**:
   ...
   **ビューポート固定（必須）:** 撮影前に `Emulation.setDeviceMetricsOverride` で解像度を固定する（CDP 接続ごとに1回）:
   ```
   Emulation.setDeviceMetricsOverride({
     width: 1920, height: 1080, deviceScaleFactor: 2, mobile: false
   })
   ```
   → 16:9、実ピクセル 3840x2160 で統一。詳細は [references/cdp-data-acquisition.md](references/cdp-data-acquisition.md#ビューポート固定必須) 参照。
